...
class CPU {
    ...
    clock() {
        ...
        if (op_code === undefined) {
            this.last_instruction = `unknown opcode: ${hex(pc)}`;
            this.halted = true;
            return;
        }
        ...
        if (instruction === undefined) {
            this.last_instruction = `undefined instruction for opcode ${mnemonic}`;
            this.halted = true;
            return;
        }
    ...
    set_zero(operand) {
        this.status = bit_mask(this.status, status_register.ZERO, operand === 0x00);
    }
    ...
    set_negative(operand) {
        this.status = bit_mask(this.status, status_register.NEGATIVE, (operand & 0b10000000) !== 0x00);
    }
